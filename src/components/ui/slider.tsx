    ...
    >
      ...
      >
        <SliderPrimitive.Range
          ...
          className={cn(
            "bg-white group-hover:bg-green-400 absolute data-[orientation=horizontal]:h-full data-[orientation=vertical]:w-full"
          )}
        />
      </SliderPrimitive.Track>
      {Array.from({ length: _values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={index}
          className="bg-white block size-4 shrink-0 rounded-full shadow-sm focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50 opacity-0 group-hover:opacity-100 transition-all duration-200 ease-in-out"
        />
      ))}
    </SliderPrimitive.Root>
  )
